...
function doPost(e) {
  try {
    ...
    var instructionText = '添付は訪問看護指示書などの医療機関発行の指示書です。記載内容から、スキーマに定義された全項目を読み取ってください。読み取れない、記載がない、または自信が持てない項目はnull(配列項目は空のnull)にし、yomitori_bikoにその旨を記載してください。';
